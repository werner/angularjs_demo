...
Taxonomy.insertTaxonomy = function(data)
{
	...
	stmt.run(null, data.title, data.code, data.taxonomy_id);
	stmt.finalize();
}
 
Taxonomy.listTaxonomies = function(callback) {
    db.all('SELECT id, title, code, taxonomy_id FROM taxonomies', function(err, rows) {
        if (err)
        {
            throw err;
        }
...
Taxonomy.showTaxonomy = function(id, callback) {
	stmt = db.prepare("SELECT id, title, code, taxonomy_id FROM taxonomies WHERE id = ?");
    stmt.bind(id); 
